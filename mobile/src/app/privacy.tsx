...
const sections = [
  { title: '食品数据从哪来', body: '澳洲主库来自 FSANZ AUSNUT 2023 与 AFCD Release 3，按官方 Data User Licence 随包使用，并附数据局限性声明。海外对照来自 USDA FoodData Central Foundation Foods（CC0），只作比较，不替代澳洲值。中餐家常菜仍是配方估算。超市包装摘录与可选「食品资料包」来自 Open Food Facts（ODbL 1.0），不是 Woolworths 或 Coles 官方商品库。资料包需你在 Wi‑Fi 下手动下载并校验后才启用，不会打进安装包。购买仍以包装标签为准。' },
  { title: '当前不做什么', body: '不会从手机调用 Open Food Facts 或超市搜索 API；不会静默自动更新资料包；不会读取通讯录、定位、相册或健康平台数据；不会把饮食记录、反馈或本地指标上传到服务器，也不会用它建立广告画像。' },
